...
class FFmpegProcessor:
    ...
    # for simplicity, we'll just do a single conversion method that takes quality settings — no separate resizing step (that can be added later if needed)
    @staticmethod
    def convert_to_mp4(
        input_path: str,
        output_path: str,
        ...
        """
        Convert video to MP4 format (no resizing — that's done in create_quality_version)
        """
        try:
            quality_settings = {
    ...
    def create_quality_version(
        ...
        """Create a resized quality version (1080p, 720p, 480p)"""
        try:
            stream = ffmpeg.input(input_path)
            stream = ffmpeg.output(
                stream,
    ...
    def generate_hls_stream(
        ...
        try:
            ...
            stream = ffmpeg.output(
                stream,
                playlist_path,              # output to playlist, not segment_pattern
                format='hls',
                hls_time=segment_time,
                hls_playlist_type='vod',
                hls_segment_filename=segment_pattern,
                vcodec='libx264',           # explicit codec for multi-thread support
                preset='fast',
                acodec='aac',
                movflags='faststart',
                **{'threads': 0}            # use all CPU cores
